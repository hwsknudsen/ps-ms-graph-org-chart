...
$users = $usersp | Sort-Object -Property UserPrincipalName
...
Foreach ($user in $users){
    ...
    if($manager.Count -eq 0){
        if((Get-MgUserDirectReport -UserId $user.UserPrincipalName).count -gt 0){
            
            Write-Output "----------ROOT Org chart for $($user.UserPrincipalName) below---------"
            write-output "$($user.DisplayName) : $($user.UserPrincipalName) : $($user.JobTitle) : $($user.Department)"

            Get-DirectReports -Manager $user.UserPrincipalName -initial -Indent $InitialIndent
            
            Write-Output "----------End of Org chart for $($user.UserPrincipalName)---------"
